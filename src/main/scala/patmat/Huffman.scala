...
object Huffman {
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      @tailrec
      def decodeAux(node: CodeTree, count: Int, result: List[Char]): List[Char] =
      {
        node match {
          case Fork(left, right, _, _) => {
            if (bits.drop(count).isEmpty) result
            else if ( bits.drop(count).head == 0 ) decodeAux(left, count + 1, result)
            else decodeAux(right, count + 1, result)
          }
          case Leaf(char, _) => decodeAux(tree, count, result ::: List(char))
        }
      }
    decodeAux(tree, 0, List())
  }
  ...
  @tailrec
  def findChar(treeAux:CodeTree, finding: Char, result:List[Bit]):List[Bit] =
  {
    treeAux match {
      case Fork(left, right, _, _) =>
        if (chars(left).contains(finding)) {
          findChar(left, finding, result ::: List(0))
        } else if (chars(right).contains(finding)){
          findChar(right, finding, result ::: List(1))
        } else Nil
      case Leaf(char, _) => if (char == finding) result else Nil
    }
  }
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      val list = for (char <- text) yield codeTable.find(x => x._1 == char).head._2
      list.flatten
    }
  }
